...
def get_electricity_step_data() -> pd.DataFrame:
    """
    Electricity per process step, per kg bead, calculated from equipment settings.
    """
    equipment_params = st.session_state.get("equipment_params", {k: dict(v) for k, v in EQUIPMENT_DEFAULTS.items()})
    _, step_df = compute_electricity_from_equipment(equipment_params)
    return step_df
...
"""
    ...
    # Sync equipment parameters from widget state and update route electricity
    sync_equipment_params_from_widgets()
    equipment_params = st.session_state["equipment_params"]
    route_elec_intensities, _ = compute_electricity_from_equipment(equipment_params)

    # Update electricity_kwh_per_fu in routes_df based on equipment scaling
    for rid, kwh_per_kg in route_elec_intensities.items():
        routes_df.loc[routes_df["route_id"] == rid, "electricity_kwh_per_fu"] = kwh_per_kg
...
"""
        ...
        route_totals, step_df = compute_electricity_from_equipment(st.session_state["equipment_params"])

        st.markdown("### Electricity intensity per kilogram of bead (current settings)")
        summary_elec = pd.DataFrame(
            {
                "Bead": ["Ref-Bead (Polymer)", "U@Bead (MOF-Functionalised)"],
                "Electricity kWh/kg": [
                    route_totals.get(ID_REF, np.nan),
                    route_totals.get(ID_MOF, np.nan),
                ],
            }
        )
        st.dataframe(summary_elec.style.format({"Electricity kWh/kg": "{:.2e}"}))

        st.markdown("### Step-level electricity breakdown (kWh per kg bead)")
        st.dataframe(step_df.style.format({"kWh_per_kg": "{:.2e}"}))
